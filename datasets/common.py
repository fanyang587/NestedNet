...
class Dataset:
    ...
    def init_input_pipeline(self, config):
        ...
        gen_function_sh, _, _ = self.get_batch_gen('search', config)
        map_func = self.get_tf_mapping(config)
        ...
        ######search
        self.sh_data = tf.data.Dataset.from_generator(gen_function_sh,
                                                       gen_types,
                                                       gen_shapes)

        # Transform inputs
        self.sh_data = self.sh_data.map(map_func=map_func, num_parallel_calls=self.num_threads)

        # Prefetch data
        self.sh_data = self.sh_data.prefetch(10)
        ...
        self.sh_init_op = iter.make_initializer(self.sh_data)
