...
from models.KPHCNN_model_stage6 import KernelPointFCNN

# Datasets
from datasets.ModelNet40 import ModelNet40Dataset
from datasets.ShapeNetPart import ShapeNetPartDataset
from datasets.S3DIS_org import S3DISDataset
...
def test_caller(path, step_ind, on_val):
    ...
    ###########################
    hw = np.load('searched_weights/stage1_hw.npy')
    mw = np.load('searched_weights/stage1_mw.npy')
    lw = np.load('searched_weights/stage1_lw.npy')

    hw2 = np.load('searched_weights/stage2_hw.npy')
    mw2 = np.load('searched_weights/stage2_mw.npy')
    lw2 = np.load('searched_weights/stage3_lw.npy')

    h2m = np.load('searched_weights/h2m.npy')
    m2l = np.load('searched_weights/m2l.npy')
    ...
    if config.dataset.startswith('ShapeNetPart'):
        model = KernelPointFCNN(dataset.flat_inputs, config, hw,mw,lw, hw2,mw2,lw2)
    elif config.dataset.startswith('S3DIS'):
        model = KernelPointFCNN(dataset.flat_inputs, config, hw,mw,lw, hw2,mw2,lw2,h2m,m2l)
    ...
    chosen_snap = os.path.join(path, 'best', 'snap91.6-233155')
...
if __name__ == '__main__':
    ...
    if chosen_log in handled_logs:
        ...
        logs = np.sort([os.path.join('./weights', f)
                        for f in os.listdir('./weights') if f.startswith('Scannet')])
